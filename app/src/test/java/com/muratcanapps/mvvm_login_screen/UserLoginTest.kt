...
class UserLoginTest {
    ...
    fun `User login when user enter correct email and password return true`() {
        ...
        val liveData = MutableLiveData<SignInWithEmailResponse>()
        val loginScreenViewModel = LoginScreenViewModel(Application())


        Mockito.`when`(loginScreenViewModel.loginAuth(email, password)).thenReturn(testSingle)
